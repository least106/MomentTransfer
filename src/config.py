...
class SystemConfig:
    ...
    def from_dict(cls, config_dict: Dict[str, Any]) -> "SystemConfig":
        ...
        cache_config = CacheConfig(**{k: v for k, v in config_dict.get("cache", {}).items() if k in cache_fields})
        ...
        physics_config = PhysicsConfig(
            **{k: v for k, v in config_dict.get("physics", {}).items() if k in {f.name for f in fields(PhysicsConfig)}}
        )
        plugin_config = PluginConfig(
            **{k: v for k, v in config_dict.get("plugin", {}).items() if k in {f.name for f in fields(PluginConfig)}}
        )
