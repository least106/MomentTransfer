...
class CompatibilityManager:
    ...
    def setup_legacy_aliases(self):
        ...
        try:
            ...
            self.main_window.src_part_name = (
                self.main_window.source_panel.part_name_input
            )
            ...
            self.main_window.btn_remove_source_part = (
                self.main_window.source_panel.btn_remove_part
            )

            # Target 面板控件引用
            self.main_window.tgt_part_name = (
                self.main_window.target_panel.part_name_input
            )
            ...
            self.main_window.btn_remove_target_part = (
                self.main_window.target_panel.btn_remove_part
            )

            # Config 面板按钮引用
            # “加载配置”入口已统一到文件列表右上角，config_panel 内已不再提供 btn_load。
            try:
                self.main_window.btn_load = getattr(
                    getattr(self.main_window, "batch_panel", None),
                    "btn_load_config",
                    None,
                )
            except Exception:
