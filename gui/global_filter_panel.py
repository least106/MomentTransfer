...
from PySide6.QtWidgets import (
    ...
    QLineEdit, QLabel, QFrame, QScrollArea, QCheckBox
)
...
class FilterCondition:
    ...
    def __init__(self, column: str, operator: FilterOperator, value: str, logic: FilterLogic = FilterLogic.AND):
        """
        Args:
            column: 列名
            operator: 比较操作符
...
class FilterConditionWidget(QWidget):
    ...
    def __init__(self, logic: FilterLogic = FilterLogic.AND, is_first: bool = False, 
                 on_remove=None, parent=None):
        super().__init__(parent)
        self.logic = logic
    ...
    def _init_ui(self):
        ...
        if not self.is_first:
            ...
            self.cmb_logic.addItems([l.value for l in FilterLogic])
    ...
    def get_condition(self) -> Optional[FilterCondition]:
        ...
        operator = next((op for op in FilterOperator if op.value == operator_text), FilterOperator.EQUALS)
        
        value = self.inp_value.text()
        
        if self.cmb_logic:
            logic_text = self.cmb_logic.currentText()
            logic = next((l for l in FilterLogic if l.value == logic_text), FilterLogic.AND)
        else:
            logic = FilterLogic.AND
