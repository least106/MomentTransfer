...
if _HAS_MPL:
    ...
    class Mpl3DCanvas(FigureCanvas):
        ...
        def plot_systems(
            ...
            # 自动调整显示范围
            all_points = np.array(
                [source_orig, target_orig, moment_center], dtype=float
            )
            ...
            if max_span < eps:
                max_range = 2.0
                logger.warning(
                    "coordinate systems are nearly coincident; using default visualization range."
                )
            else:
    ...
    class Mpl3DCanvas:
        def __init__(self, *args, **kwargs):
            raise RuntimeError(
                "matplotlib not available or failed to initialize; 3D canvas is disabled"
            )
