...
class QuickSelectDialog(QDialog):
    ...
    def _add_entry(
        self, fp_str: str, part: Optional[str], kstr: Optional[str] = None
    ) -> None:
        ...
        entry = {
            "key": (fp_str, part),
            "kstr": kstr or make_qs_key(fp_str, part),
            "input": inp,
            "preview": preview,
        }
            ...
            for entry in list(self._entry_widgets or []):
                try:
                    ...
                    if key:
                        ...
                        try:
                            entry_fp = key[0]
                            if normalize_path_key(fp_str) == normalize_path_key(
                                entry_fp
                            ):
                                self._update_entry_preview(entry)
                        except Exception:
            ...
            for entry in self._entry_widgets:
                ...
                if rows is None:
                    if part is None:
                        ...
                        row_count = len(df) if df is not None else 0
                        by_file = (
                            getattr(gui, "special_part_row_selection_by_file", {}) or {}
                        )
            ...
            for i in range(self.tree.topLevelItemCount()):
                ...
                try:
                    ...
                    if part is None:
                        ...
                        if sel is not None:
                            it.setCheckState(0, Qt.Checked)
                            data = self.batch._get_special_data_dict(Path(fp_norm))
                            df = (data or {}).get(str(part))
                            row_count = len(df) if df is not None else 0
                            skipped = []
                            for r in range(row_count):
