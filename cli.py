...
@click.option('--force', type=(float, float, float), required=True,
              help='输入力向量，格式: Fx Fy Fz (例: --force 100 0 -50)')
@click.option('--moment', type=(float, float, float), required=True,
              help='输入力矩向量，格式: Mx My Mz (例: --moment 0 500 0)')
@click.option('--target-part', default=None,
              help='目标 part 名称（可选，多 part 时指定）')
@click.option('--target-variant', type=int, default=0,
              help='目标 variant 索引，默认 0')
