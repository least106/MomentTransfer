...
def _worker_write(args):
    ...
    processed, dropped, n_non, _ = b.process_df_chunk(df, 0, 1, 2, 3, 4, 5, calculator, cfg, out_path, False, logger)
    return processed
...
def test_concurrent_appends_to_same_file(tmp_path):
    ...
    assert project_config.exists(), "需要 repository 中的 data/input.json 用于构造计算器"
    ...
    b.process_df_chunk(first_df, 0, 1, 2, 3, 4, 5, calculator, cfg, out_file, True, logger)

    # 并发提交多个写入任务（模拟 4 个并发进程）
    workers = 4
    args = {'project_path': str(project_config), 'input_csv': str(input_csv), 'out_path': str(out_file)}
    results = []
