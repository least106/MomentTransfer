...
def _worker_write(args):
    ...
    project_data, calculator = load_project_calculator(
        project_path, target_part="TestModel"
    )
    ...
    processed, dropped, n_non, _ = b.process_df_chunk(
        df, calculator, cfg, out_path, False, logger
    )
    return processed
...
def test_concurrent_appends_to_same_file(tmp_path):
    ...
    df_in.to_csv(input_csv, index=False, header=True)
    ...
    project_data, calculator = load_project_calculator(
        project_config, target_part="TestModel"
    )
    # 主进程也设置 cfg
    calculator.cfg = project_data
    cfg = BatchConfig()
