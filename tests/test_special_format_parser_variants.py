...
def make_frame_for_part(name="P", q=100.0, s=10.0):
    ...
    return FrameConfiguration(part_name=name, coord_system=coord, moment_center=[0,0,0], c_ref=1.0, b_ref=1.0, q=q, s_ref=s)
...
def test_header_variants_czfn_and_cmx(tmp_path):
    ...
    proj = ProjectData(source_parts={"src":[frame]}, target_parts={"WingV":[frame]})

    outputs, report = sfp.process_special_format_file(p, proj, tmp_path, return_report=True)
...
def test_lowercase_and_underscore_variants(tmp_path):
    ...
    proj = ProjectData(source_parts={"src":[frame]}, target_parts={"PartL":[frame]})
