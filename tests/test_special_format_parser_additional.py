...
def test_process_special_format_file_success(tmp_path):
    ...
    outputs, report = sfp.process_special_format_file(p, proj, out_dir, return_report=True)
...
def test_process_special_format_file_skip_missing_target(tmp_path):
    ...
    assert report and report[0]['status'] == 'skipped' and report[0]['reason'] == 'target_missing'
