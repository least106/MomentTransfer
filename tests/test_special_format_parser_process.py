...
def test_process_special_format_file_success(tmp_path: Path, monkeypatch):
    ...
    outputs, report = sfp.process_special_format_file(p, project_data, outdir, return_report=True, overwrite=True)
...
def test_process_special_format_file_no_project_data(tmp_path: Path):
    ...
    outputs, report = sfp.process_special_format_file(p, None, outdir, return_report=True)
    assert outputs == []
...
def test_process_special_format_file_missing_columns(tmp_path: Path):
    ...
    outputs, report = sfp.process_special_format_file(p, project_data, outdir, return_report=True)
    assert outputs == []
