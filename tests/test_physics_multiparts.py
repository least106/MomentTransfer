...
def _make_frame_dict(name="P", origin=(0, 0, 0)):
    return {
        "PartName": name,
        "CoordSystem": {"Orig": list(origin), "X": [1, 0, 0], "Y": [0, 1, 0], "Z": [0, 0, 1]},
        "MomentCenter": [0, 0, 0],
        "Q": 1.0,
        "S": 1.0,
    }
...
def test_aerocalc_warns_on_multiple_target_parts():
    ...
    data = {
        "Source": {"Parts": [{"PartName": "src", "Variants": [{**_make_frame_dict("src")}]}]},
        "Target": {
            "Parts": [
...
def test_load_project_calculator_logs_warning_for_multiple_targets(tmp_path, caplog):
    ...
    assert any("未指定 --target-part" in rec.getMessage() or "自动选择第一个 Part" in rec.getMessage() for rec in caplog.records)
