...
class TestHistoryIsolation:
    ...
    def test_history_store_uses_temp_path(self):
        ...
        assert str(store.store_path).startswith(str(temp_dir)), \
    ...
    def test_production_path_unchanged(self):
        ...
        test_records = [
            r for r in records 
            if "test_input.csv" in r.get("input_path", "") or
               "test_file.csv" in str(r.get("files", []))
        ]
    ...
    def test_batch_manager_respects_testing_flag(self):
        ...
        if batch_manager.history_store:
            assert batch_manager.history_store._is_testing is True, \
