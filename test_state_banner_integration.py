...
def test_state_manager_import():
    ...
    try:
        ...
        # 测试初始状态
        assert sm1.current_state == AppState.NORMAL, f"初始状态应为NORMAL，但为 {sm1.current_state}"
        logger.info("✓ 初始状态正确")
...
def test_signal_emissions():
    ...
    try:
        ...
        def on_redo_mode_changed(is_entering, record_id):
            ...
            logger.debug(f"信号: redoModeChanged -> is_entering={is_entering}, record_id={record_id}")
        ...
        # 验证信号发送
        assert any(e[0] == 'redoModeChanged' and e[1] == True for e in signal_emissions), \
        ...
        assert any(e[0] == 'redoModeChanged' and e[1] == False for e in signal_emissions), \
