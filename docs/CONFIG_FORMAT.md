...
6. **路径与平台兼容性**
  - 配置文件中请尽量使用相对路径（例如 `../data`），避免写入操作系统专有的绝对路径（如 `C:\\...` 或 `/home/...`）。
  - 在代码中解析配置路径时，优先使用 `pathlib.Path` 或 `os.path.join` 来构造路径，避免直接拼接字符串。
  - 如果必须在示例中说明绝对路径，请在注释中标注该路径为示例，仅供说明，且在跨平台使用前应替换为相对路径或由用户配置。
